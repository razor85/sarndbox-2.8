...
RemoteServer::Client::Client(RemoteServer *sServer)
    : server(sServer), clientPipe(server->listenSocket), state(START) {}
...
void RemoteServer::disconnectClient(Client *client, bool removeListener) {
  ...
  for (std::vector<Client *>::iterator cIt = clients.begin();
       cIt != clients.end(); ++cIt)
    if (*cIt == client) {
...
bool RemoteServer::newConnectionCallback(
  ...
  try {
    ...
    newClient->listenerKey = thisPtr->dispatcher.addIOEventListener(
      newClient->clientPipe.getFd(), Threads::EventDispatcher::Read,
      thisPtr->clientMessageCallback, newClient);
...
bool RemoteServer::clientMessageCallback(
  Threads::EventDispatcher::ListenerKey eventKey, int eventType,
  void *userData) {
  ...
  try {
    ...
    Misc::formattedConsoleWarning(
      "RemoteServer: Disconnecting client due to exception %s", err.what());
...
void *RemoteServer::communicationThreadMethod(void) {
  /* Dispatch events on the communications socket(s) until stopped by the main
   * thread: */
  while (dispatcher.dispatchNextEvent()) {
    /* Collect the current positions of all connected clients in streaming
     * state: */
    std::vector<Vrui::ONTransform> &positions = clientPositions.startNewValue();
    positions.clear();
    Vrui::Point gridOffset(Math::div2(gridSize[0] * cellSize[0]),
      Math::div2(gridSize[1] * cellSize[1]));
    for (std::vector<Client *>::iterator cIt = clients.begin();
         cIt != clients.end(); ++cIt)
      if ((*cIt)->state == Client::STREAMING) {
        Vrui::Vector t = (*cIt)->position - gridOffset;
        Vrui::Rotation r = Vrui::Rotation::rotateFromTo(Vrui::Vector(0, 0, -1),
          (*cIt)->direction);
    ...
    if (grids.lockNewValue()) {
      ...
      for (std::vector<Client *>::iterator cIt = clients.begin();
        ...
        if ((*cIt)->state == Client::STREAMING) {
          ...
          try {
            ...
            /* Disconnect the client: */
            Misc::formattedConsoleWarning(
              "RemoteServer: Disconnecting client due to exception %s",
              err.what());
      ...
      for (std::vector<Client *>::iterator dcIt = deadClients.begin();
           dcIt != deadClients.end(); ++dcIt)
...
void RemoteServer::readBackCallback(GLfloat *, GLfloat *, void *userData) {
  ...
  /* Post the new grids to the grid triple buffer and wake up the communication
   * thread: */
  thisPtr->grids.postNewValue();
  thisPtr->dispatcher.interrupt();
}

RemoteServer::RemoteServer(Sandbox *sSandbox, int listenPortId,
  double sRequestInterval)
    : sandbox(sSandbox), listenSocket(listenPortId, 0), numClients(0),
  ...
  dispatcher.addIOEventListener(listenSocket.getFd(),
    Threads::EventDispatcher::Read, newConnectionCallback, this);
...
void RemoteServer::frame(double applicationTime) {
  ...
  if (numClients > 0 && applicationTime >= nextRequestTime) {
    ...
    if (sandbox->gridRequest.requestGrids(gb.bathymetry, gb.waterLevel,
      ...
      nextRequestTime = (Math::floor(applicationTime / requestInterval) + 1.0) *
        requestInterval;
    }
  }
}
...
void RemoteServer::glRenderAction(GLContextData &contextData) const {
  ...
  const std::vector<Vrui::ONTransform> &positions = clientPositions
                                                      .getLockedValue();
  if (!positions.empty()) {
