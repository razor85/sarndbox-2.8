...
void *FrameFilter::filterThreadMethod(void) {
  ...
  while (true) {
    ...
    /* Enter the new frame into the averaging buffer and calculate the output
     * frame's pixel values: */
    ...
    for (unsigned int y = 0; y < size[1]; ++y) {
      ...
      for (unsigned int x = 0; x < size[0];
        ...
        /* Plug the depth-corrected new value into the minimum and maximum plane
         * equations to determine its validity: */
        float minD = minPlane[0] * px + minPlane[1] * py +
          minPlane[2] * newCVal + minPlane[3];
        float maxD = maxPlane[0] * px + maxPlane[1] * py +
          maxPlane[2] * newCVal + maxPlane[3];
        ...
        if (sPtr[0] >= minNumSamples &&
          sPtr[2] * sPtr[0] <=
            maxVariance * sPtr[0] * sPtr[0] + sPtr[1] * sPtr[1]) {
          /* Check if the new depth-corrected running mean is outside the
           * previous value's envelope: */
...
FrameFilter::FrameFilter(const unsigned int sSize[2],
  ...
  const PTransform &depthProjection, const Plane &basePlane)
    : pixelDepthCorrection(sPixelDepthCorrection), averagingBuffer(0),
      statBuffer(0), outputFrameFunction(0) {
  ...
  PTransform::HVector basePlaneDic(
    depthProjection.getMatrix().transposeMultiply(basePlaneCc));
  ...
  for (unsigned int y = 0; y < size[1]; ++y)
    for (unsigned int x = 0; x < size[0]; ++x, ++vbPtr)
      *vbPtr = float(-((double(x) + 0.5) * basePlaneDic[0] +
                       (double(y) + 0.5) * basePlaneDic[1] + basePlaneDic[3]) /
        basePlaneDic[2]);
...
void FrameFilter::setValidDepthInterval(unsigned int newMinDepth,
...
void FrameFilter::setValidElevationInterval(const PTransform &depthProjection,
  ...
  minPlaneCc[3] = -(
    basePlane.getOffset() + newMinElevation * basePlane.getNormal().mag());
  PTransform::HVector maxPlaneCc(basePlane.getNormal());
  maxPlaneCc[3] = -(
    basePlane.getOffset() + newMaxElevation * basePlane.getNormal().mag());
...
void FrameFilter::setStableParameters(unsigned int newMinNumSamples,
...
void FrameFilter::setOutputFrameFunction(
  FrameFilter::OutputFrameFunction *newOutputFrameFunction) {
  delete outputFrameFunction;
