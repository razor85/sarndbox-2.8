...
DEMToolFactory::DEMToolFactory(Vrui::ToolManager &toolManager)
    : ToolFactory("DEMTool", toolManager),
      demSelectionHelper(Vrui::getWidgetManager(), "", ".grid",
        IO::openDirectory(".")) {
...
const char *DEMToolFactory::getName(void) const { return "Show DEM"; }

const char *DEMToolFactory::getButtonFunction(int) const {
  return "Toggle DEM";
}

Vrui::Tool *DEMToolFactory::createTool(
  const Vrui::ToolInputAssignment &inputAssignment) const {
  return new DEMTool(this, inputAssignment);
}

void DEMToolFactory::destroyTool(Vrui::Tool *tool) const { delete tool; }
...
void DEMTool::loadDEMFile(const char *demFileName) {
  ...
  if (haveDemTransform)
    ...
    /* Calculate an appropriate DEM transformation to fit the DEM into the
     * sandbox's domain: */
    ...
    if (scale < scaleRot) {
      ...
      demT *= OGTransform::rotate(
        OGTransform::Rotation::rotateZ(Math::rad(Scalar(90))));
      scale = scaleRot;
    }
  ...
  demT *= OGTransform::translate(
    Vector(0, 0, demVerticalShift / demVerticalScale));

  /* Set the DEM transformation: */
  setTransform(demT * OGTransform(application->boxTransform), demVerticalScale,
    demT.getOrigin()[2]);
}

void DEMTool::loadDEMFileCallback(
  GLMotif::FileSelectionDialog::OKCallbackData *cbData) {
  /* Load the selected DEM file: */
  loadDEMFile(
    cbData->selectedDirectory->getPath(cbData->selectedFileName).c_str());
}
...
DEMTool::DEMTool(const Vrui::ToolFactory *factory,
  const Vrui::ToolInputAssignment &inputAssignment)
    : Vrui::Tool(factory, inputAssignment), haveDemTransform(false),
      demTransform(OGTransform::identity), demVerticalShift(0),
      demVerticalScale(1) {}

DEMTool::~DEMTool(void) {}

void DEMTool::configure(
  ...
  if (configFileSection.hasTag("./demTransform")) {
    ...
    demTransform = configFileSection.retrieveValue<OGTransform>(
      "./demTransform", demTransform);
  }

  demVerticalShift = configFileSection.retrieveValue<Scalar>(
    "./demVerticalShift", demVerticalShift);
  demVerticalScale = configFileSection.retrieveValue<Scalar>(
...
void DEMTool::initialize(void) {
  ...
  if (demFileName.empty()) {
    /* Load a DEM file: */
    factory->demSelectionHelper.loadFile("Load DEM File...", this,
      &DEMTool::loadDEMFileCallback);
  } else {
...
const Vrui::ToolFactory *DEMTool::getFactory(void) const { return factory; }

void DEMTool::buttonCallback(int buttonSlotIndex,
