...
DEM::DataItem::DataItem(void) : textureObjectId(0) {
...
void DEM::calcMatrix(void) {
  ...
  /* Pre-multiply the projective transformation matrix with the DEM space to DEM
   * pixel space transformation: */
  PTransform dem;
  dem.getMatrix()(0, 0) = Scalar(demSize[0] - 1) / (demBox[2] - demBox[0]);
  dem.getMatrix()(0, 3) = Scalar(0.5) -
    Scalar(demSize[0] - 1) / (demBox[2] - demBox[0]) * demBox[0];
  dem.getMatrix()(1, 1) = Scalar(demSize[1] - 1) / (demBox[3] - demBox[1]);
  dem.getMatrix()(1, 3) = Scalar(0.5) -
    Scalar(demSize[1] - 1) / (demBox[3] - demBox[1]) * demBox[1];
...
DEM::DEM(void)
    : dem(0), transform(OGTransform::identity), verticalScale(1),
      verticalScaleBase(0) {
  demSize[0] = demSize[1] = 0;
}

DEM::~DEM(void) { delete[] dem; }
...
void DEM::initContext(GLContextData &contextData) const {
  ...
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_LUMINANCE32F_ARB, demSize[0],
    demSize[1], 0, GL_LUMINANCE, GL_FLOAT, dem);
