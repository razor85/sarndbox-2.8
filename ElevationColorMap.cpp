  ...
  if (Misc::hasCaseExtension(heightMapName, ".cpt")) {
    ...
    while (!heightMapSource.eof()) {
      ...
      if (!heightMapSource.isLiteral('\n'))
        Misc::throwStdErr(
          "ElevationColorMap: Color map format error in line %d of file %s",
          line, fullHeightMapName.c_str());
      ++line;
    }
  } else {
    heightMapSource.setPunctuation(",\n");
...
void ElevationColorMap::calcTexturePlane(const Plane &basePlane) {
  ...
  Scalar hms = Scalar(getNumEntries() - 1) /
    ((getScalarRangeMax() - getScalarRangeMin()) * Scalar(getNumEntries()));
  Scalar hmo = Scalar(0.5) / Scalar(getNumEntries()) -
    hms * getScalarRangeMin();
  for (int i = 0; i < 3; ++i)
    texturePlaneEq[i] = GLfloat(bpn[i] * hms);
  texturePlaneEq[3] = GLfloat(-bpo * hms + hmo);
}

void ElevationColorMap::calcTexturePlane(
  const DepthImageRenderer *depthImageRenderer) {
  /* Calculate texture plane based on the given depth image renderer's base
   * plane: */
...
void ElevationColorMap::bindTexture(GLContextData &contextData) const {
  ...
  if (dataItem->textureObjectVersion != textureVersion) {
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB8, getNumEntries(), 0, GL_RGBA,
      GL_FLOAT, getColors());
