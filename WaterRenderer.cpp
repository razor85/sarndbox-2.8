...
WaterRenderer::DataItem::DataItem(void)
    : vertexBuffer(0), indexBuffer(0), waterShader(0) {
...
WaterRenderer::WaterRenderer(const WaterTable2 *sWaterTable)
  ...
  /* Calculate the transposed tangent-plane transformation from grid space to
   * world space: */
...
void WaterRenderer::initContext(GLContextData &contextData) const {
  ...
  glBufferDataARB(GL_ARRAY_BUFFER_ARB,
    waterGridSize[1] * waterGridSize[0] * sizeof(Vertex), 0,
    GL_STATIC_DRAW_ARB);
  ...
  glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
    (waterGridSize[1] - 1) * waterGridSize[0] * 2 * sizeof(GLuint), 0,
    GL_STATIC_DRAW_ARB);
  ...
  *(ulPtr++) = glGetUniformLocationARB(dataItem->waterShader,
    "quantitySampler");
  *(ulPtr++) = glGetUniformLocationARB(dataItem->waterShader,
    "bathymetrySampler");
  *(ulPtr++) = glGetUniformLocationARB(dataItem->waterShader,
...
void WaterRenderer::render(const PTransform &projection,
  ...
  /* Calculate and upload the vertex transformation from grid space to eye
   * space: */
  ...
  const Scalar *tmvgtPtr = tangentModelviewGridTransform.getMatrix()
                             .getEntries();
  ...
  glUniformMatrix4fvARB(*(ulPtr++), 1, GL_FALSE,
    tangentModelviewGridTransformMatrix);

  /* Calculate and upload the vertex transformation from grid space to clip
   * space: */
  ...
  GLuint *indexPtr = 0;
  for (unsigned int y = 1; y < waterGridSize[1];
       ++y, indexPtr += waterGridSize[0] * 2)
    glDrawElements(GL_QUAD_STRIP, waterGridSize[0] * 2, GL_UNSIGNED_INT,
      indexPtr);
