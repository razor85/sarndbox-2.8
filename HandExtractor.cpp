...
struct BlobOrigin // Helper structure to store a point on the border of a
                  // foreground blob
{
  /* Elements: */
public:
...
HandExtractor::HandExtractor(const unsigned int sDepthFrameSize[2],
  ...
  const PTransform &sDepthProjection)
    : pixelDepthCorrection(sPixelDepthCorrection),
      depthProjection(sDepthProjection), inputFrameVersion(0),
      runExtractorThread(false), maxFgDepth(0x07ffU - 1U), maxDepthDist(1),
      minBlobSize(1500), maxBlobSize(150000), blobIdImage(0), snakeLength(50),
      snake(0), maxCornerEnterDist(28), minCenterDist(10),
      minCornerExitDist(32), minHandProbability(0.15f),
  ...
  blobIdImage = new unsigned short[(depthFrameSize[1] + 2) *
    (depthFrameSize[0] + 2)];
...
void HandExtractor::setBlobSizeRange(unsigned int newMinBlobSize,
...
void HandExtractor::setCornerDists(int newMaxCornerEnterDist,
  int newMinCenterDist, int newMinCornerExitDist) {
...
void HandExtractor::extractHands(const HandExtractor::DepthPixel *depthFrame,
  ...
  for (unsigned int y = 0; y < depthFrameSize[1];
       ++y, dfRowPtr += depthFrameSize[0]) {
    ...
    while (true) {
      ...
      ++x;
      ++dfPtr;
      for (; x < depthFrameSize[0] && *dfPtr <= maxFgDepth &&
           *dfPtr + maxDepthDist >= lastDepth &&
           *dfPtr <= lastDepth + maxDepthDist;
           ++x, ++dfPtr)
        lastDepth = *dfPtr;
      ...
      for (; lastRowSpan < rowSpan && spans[lastRowSpan].end < newSpan.start;
           ++lastRowSpan)
        ;

      /* Check if the current span links up with any from the previous row: */
      for (unsigned int lrs = lastRowSpan;
           lrs < rowSpan && spans[lrs].start <= newSpan.end; ++lrs) {
        ...
        for (unsigned int o = o1; o < o2 && !canLink; ++o, ++lrsPtr0, ++lrsPtr1)
          canLink = *lrsPtr0 + maxDepthDist >= *lrsPtr1 &&
            *lrsPtr0 <= *lrsPtr1 + maxDepthDist;
  ...
  for (unsigned int i = 0; i < numSpans; ++i) {
    ...
    if (spans[i].parent == i) {
      if (spans[i].numPixels >= minBlobSize &&
        spans[i].numPixels <= maxBlobSize) {
  ...
  for (unsigned int y = 0; y < depthFrameSize[1]; ++y, biRowPtr += biStride) {
    ...
    while (true) {
      ...
      /* Check if the current span's blob is valid, and encountered for the
       * first time: */
  ...
  /* Walk around the edges of all foreground blobs in counter-clockwise order
   * and decide whether they are hand-shaped: */
  ...
  for (unsigned int blobId = 0; blobId < nextBlobId; ++blobId) {
    ...
    unsigned int
      walkDir = 0; // The blob origin is the bottom-left pixel of the blob, so 0
                   // is the correct initial walking direction
    ...
    do {
      ...
      int newCornerType = 0;
      int headTailDist2 = Math::sqr(snakeHead->x - snakeTail->x) +
        Math::sqr(snakeHead->y - snakeTail->y);
      int centerElevation2 = 0;
      if (headTailDist2 <= enterDist2) {
        /* Determine the type of corner by comparing the snake's center point
         * against the line defined by its head and tail: */
        int nx = snakeTail->y - snakeHead->y;
        int ny = snakeHead->x - snakeTail->x;
        int d = nx * (snakeMid->x - snakeTail->x) +
          ny * (snakeMid->y - snakeTail->y);
        if (Math::sqr(d) >= centerDist2 * headTailDist2) {
          ...
          if (headTailDist2 > 0)
            ...
            centerElevation2 = Math::sqr(snakeMid->x - snakeTail->x) +
              Math::sqr(snakeMid->y - snakeTail->y);
        }
      }
      ...
      if (imgPtr != 0) {
        ...
        Images::RGBImage::Color *cPtr = imgPtr +
          (snakeMid->y * depthFrameSize[0] + snakeMid->x);
    ...
    if (corner.cornerType != 0) {
      if (!corners.empty() && firstCornerStart == 0 &&
        corners.front().cornerType == corner.cornerType) {
    ...
    if (imgPtr != 0) {
      /* Draw all corners: */
      for (std::vector<Corner>::iterator cIt = corners.begin();
           cIt != corners.end(); ++cIt) {
        Images::RGBImage::Color *cPtr = imgPtr +
          (cIt->y * depthFrameSize[0] + cIt->x);
        if (cIt->cornerType == 1)
    ...
    if (numCorners >= 8) // At least four finger tips, three nooks, and a thumb
                         // tip (thumb nook optional)
    {
      for (size_t i = 0; i < numCorners; ++i) {
        /* Check if the current corner starts a sequence of four tips
         * interleaved with three nooks: */
        Corner &t0 = corners[i];
        ...
        if (t0.cornerType == 1 && n1.cornerType == -1 && t1.cornerType == 1 &&
          n2.cornerType == -1 && t2.cornerType == 1 && n3.cornerType == -1 &&
          t3.cornerType == 1) {
          ...
          if (maxProb < prob) {
            ...
            float fdNdRatio = Math::mid(Geometry::dist(tp1, rp1),
                                Geometry::dist(tp2, rp2)) /
              Math::mid(Geometry::dist(np1, np2), Geometry::dist(np2, np3));

            /* Calculate the hand center and radius: */
            float centerOffset = 1.0f / fdNdRatio;
            center = Geometry::mid(
              Geometry::mid(rp0 + (rp0 - tp0) * centerOffset,
                rp1 + (rp1 - tp1) * centerOffset),
              Geometry::mid(rp2 + (rp2 - tp2) * centerOffset,
                rp3 + (rp3 - tp3) * centerOffset));
            center = Geometry::mid(rp1 + (rp1 - tp1) * centerOffset,
              rp2 + (rp2 - tp2) * centerOffset);
            radius = (Geometry::dist(center, tp0) +
                       Geometry::dist(center, tp1) +
                       Geometry::dist(center, tp2) +
                       Geometry::dist(center, tp3)) *
              0.25f;

            /* Calculate the hand's average depth in depth-corrected depth image
             * space: */
            depth = 0.0f;
            if (pixelDepthCorrection != 0) {
              ptrdiff_t t0Off = t0.y * depthFrameSize[0] + t0.x;
              depth += pixelDepthCorrection[t0Off].correct(
                float(depthFrame[t0Off]));
              ptrdiff_t n1Off = n1.y * depthFrameSize[0] + n1.x;
              depth += pixelDepthCorrection[n1Off].correct(
                float(depthFrame[n1Off]));
              ptrdiff_t t1Off = t1.y * depthFrameSize[0] + t1.x;
              depth += pixelDepthCorrection[t1Off].correct(
                float(depthFrame[t1Off]));
              ptrdiff_t n2Off = n2.y * depthFrameSize[0] + n2.x;
              depth += pixelDepthCorrection[n2Off].correct(
                float(depthFrame[n2Off]));
              ptrdiff_t t2Off = t2.y * depthFrameSize[0] + t2.x;
              depth += pixelDepthCorrection[t2Off].correct(
                float(depthFrame[t2Off]));
              ptrdiff_t n3Off = n3.y * depthFrameSize[0] + n3.x;
              depth += pixelDepthCorrection[n3Off].correct(
                float(depthFrame[n3Off]));
              ptrdiff_t t3Off = t3.y * depthFrameSize[0] + t3.x;
              depth += pixelDepthCorrection[t3Off].correct(
                float(depthFrame[t3Off]));
            } else {
            ...
            if (imgPtr != 0) {
              /* Draw the hand: */
              drawLine(*blobImage, tp0, rp0,
                Images::RGBImage::Color(255, 255, 255));
              drawLine(*blobImage, tp1, rp1,
                Images::RGBImage::Color(255, 255, 255));
              drawLine(*blobImage, tp2, rp2,
                Images::RGBImage::Color(255, 255, 255));
              drawLine(*blobImage, tp3, rp3,
                Images::RGBImage::Color(255, 255, 255));
              drawCircle(*blobImage, center, radius,
                Images::RGBImage::Color(255, 255, 255));
            }
          }
        }
    ...
    if (maxProb > minHandProbability) {
      // DEBUGGING
      // std::cout<<"Hand in depth space: "<<center[0]<<", "<<center[1]<<",
      // "<<depth<<", "<<radius<<std::endl;

      /* Store the hand in camera space: */
      Hand newHand;
      newHand.center = depthProjection.transform(
        Point(center[0], center[1], depth));
      newHand.radius = Geometry::dist(newHand.center,
        depthProjection.transform(Point(center[0] + radius, center[1], depth)));
      hands.push_back(newHand);

      // DEBUGGING
      // std::cout<<"Hand in camera space: "<<newHand.center[0]<<",
      // "<<newHand.center[1]<<", "<<newHand.center[2]<<",
      // "<<newHand.radius<<std::endl;
    }
...
void HandExtractor::setHandsExtractedFunction(
  HandExtractor::HandsExtractedFunction *newHandsExtractedFunction) {
