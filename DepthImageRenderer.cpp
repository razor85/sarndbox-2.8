...
DepthImageRenderer::DataItem::DataItem(void)
    : vertexBuffer(0), indexBuffer(0), depthTexture(0), depthTextureVersion(0),
...
void DepthImageRenderer::initContext(GLContextData &contextData) const {
  ...
  glBufferDataARB(GL_ARRAY_BUFFER_ARB,
    depthImageSize[1] * depthImageSize[0] * sizeof(Vertex), 0,
    GL_STATIC_DRAW_ARB);
    ...
    for (unsigned int y = 0; y < depthImageSize[1]; ++y)
      for (unsigned int x = 0; x < depthImageSize[0]; ++x, ++vPtr) {
        ...
        Kinect::LensDistortion::Point dp(Kinect::LensDistortion::Scalar(x) +
          ...
          Kinect::LensDistortion::Scalar(y) +
            Kinect::LensDistortion::Scalar(0.5));
  ...
  glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
    (depthImageSize[1] - 1) * depthImageSize[0] * 2 * sizeof(GLuint), 0,
    GL_STATIC_DRAW_ARB);
  ...
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_LUMINANCE32F_ARB,
    depthImageSize[0], depthImageSize[1], 0, GL_LUMINANCE, GL_FLOAT, 0);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, 0);

  /* Create the depth rendering shader: */
  dataItem->depthShader = linkVertexAndFragmentShader("SurfaceDepthShader");
  dataItem->depthShaderUniforms[0] = glGetUniformLocationARB(
    dataItem->depthShader, "depthSampler");
  dataItem->depthShaderUniforms[1] = glGetUniformLocationARB(
    dataItem->depthShader, "projectionModelviewDepthProjection");

  /* Create the elevation rendering shader: */
  dataItem->elevationShader = linkVertexAndFragmentShader(
    "SurfaceElevationShader");
  dataItem->elevationShaderUniforms[0] = glGetUniformLocationARB(
    dataItem->elevationShader, "depthSampler");
  dataItem->elevationShaderUniforms[1] = glGetUniformLocationARB(
    dataItem->elevationShader, "basePlaneDic");
  dataItem->elevationShaderUniforms[2] = glGetUniformLocationARB(
    dataItem->elevationShader, "weightDic");
  dataItem->elevationShaderUniforms[3] = glGetUniformLocationARB(
    dataItem->elevationShader, "projectionModelviewDepthProjection");
}

void DepthImageRenderer::setDepthProjection(
  const PTransform &newDepthProjection) {
...
void DepthImageRenderer::setIntrinsics(
  const Kinect::FrameSource::IntrinsicParameters &ips) {
...
void DepthImageRenderer::setBasePlane(const Plane &newBasePlane) {
  ...
  /* Transform the base plane to depth image space and into a GLSL-compatible
   * format: */
  ...
  for (int i = 0; i < 4; ++i)
    basePlaneDicEq[i] = GLfloat(dpm(0, i) * bpn[0] + dpm(1, i) * bpn[1] +
      dpm(2, i) * bpn[2] - dpm(3, i) * bpo);
}

void DepthImageRenderer::setDepthImage(
  const Kinect::FrameBuffer &newDepthImage) {
  /* Update the depth image: */
  depthImage = newDepthImage;
  ++depthImageVersion;
}

Scalar DepthImageRenderer::intersectLine(const Point &p0, const Point &p1,
  Scalar elevationMin, Scalar elevationMax) const {
...
void DepthImageRenderer::renderSurfaceTemplate(
  ...
  for (unsigned int y = 1; y < depthImageSize[1];
    ...
    glDrawElements(GL_QUAD_STRIP, depthImageSize[0] * 2, GL_UNSIGNED_INT,
      indexPtr);
