...
constexpr bool RenderDepthMask{ true };
...
void SurfaceRenderer::shaderSourceFileChanged(
  const IO::FileMonitor::Event &event) {
  /* Invalidate the single-pass surface shader: */
  ++surfaceSettingsVersion;
}

GLhandleARB SurfaceRenderer::createSinglePassSurfaceShader(
  const GLLightTracker &lt, GLint *uniformLocations,
  GLint *depthMaskUniform) const {
  ...
  try {
    ...
    *********************************************************************/
    ...
    if (illuminate) {
      ...
      /* Call the appropriate light accumulation function for every enabled
       * light source: */
      ...
      for (int lightIndex = 0; lightIndex < lt.getMaxNumLights(); ++lightIndex)
        if (lt.getLightState(lightIndex).isEnabled()) {
          ...
          /* Call the light accumulation function from vertex shader's main
           * function: */
          vertexMain += "\
						accumulateLight";
          char liBuffer[12];
          vertexMain.append(Misc::print(lightIndex, liBuffer + 11));
          vertexMain += "(vertexEc,normalEc,gl_FrontMaterial.ambient,gl_"
                        "FrontMaterial.diffuse,gl_FrontMaterial.specular,gl_"
                        "FrontMaterial.shininess,diffColor,specColor);\n";
        }
      if (!firstLight)
        vertexMain += "\
					\n";
    ...
    shaders.push_back(glCompileVertexShaderFromStrings(7,
      vertexFunctions.c_str(), "\t\t\n", vertexUniforms.c_str(), "\t\t\n",
    ...
    *********************************************************************/
    ...
    if constexpr (RenderDepthMask) {
     fragmentUniforms += "\n\
			...
			uniform vec2 maskDepthViewport;     // Viewport dimensions for this sampler\n\
      ";
    }
    ...
    if constexpr (RenderDepthMask) {
      fragmentMain += "\
  			...
  			float maskValue = texture2D(maskDepthSampler, gl_FragCoord.xy / maskDepthViewport.xy).a;\n\
  			gl_FragColor = baseColor * vec4(maskValue);\n\
  			}\n";
    } else {
      fragmentMain += "\
  			/* Assign the final color to the fragment: */\n\
    ...
    shaders.push_back(glCompileFragmentShaderFromStrings(7,
      fragmentDeclarations.c_str(), "\t\t\n", fragmentUniforms.c_str(),
    ...
    for (std::vector<GLhandleARB>::iterator shIt = shaders.begin();
    ...
    *(ulPtr++) = glGetUniformLocationARB(result, "depthProjection");
    ...
    if (drawContourLines) {
      *(ulPtr++) = glGetUniformLocationARB(result,
    ...
    *(ulPtr++) = glGetUniformLocationARB(result,
    ...
    if constexpr (RenderDepthMask) {
      depthMaskUniform[0] = glGetUniformLocationARB(result, "maskDepthSampler");
      depthMaskUniform[1] = glGetUniformLocationARB(result,
        "maskDepthViewport");
    }
...
void SurfaceRenderer::renderPixelCornerElevations(const int viewport[4],
  ...
  glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,
    dataItem->contourLineFramebufferObject);

  /* Check if the contour line frame buffer needs to be resized: */
  if (dataItem->contourLineFramebufferSize[0] !=
      (unsigned int)(viewport[2] + 1) ||
    dataItem->contourLineFramebufferSize[1] !=
      (unsigned int)(viewport[3] + 1)) {
    /* Remember if the render buffers must still be attached to the frame
     * buffer: */
    ...
    for (int i = 0; i < 2; ++i)
      dataItem->contourLineFramebufferSize[i] = (unsigned int)(viewport[2 + i] +
        1);
    ...
    glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT,
      dataItem->contourLineFramebufferSize[0],
      dataItem->contourLineFramebufferSize[1]);
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F,
      dataItem->contourLineFramebufferSize[0],
      dataItem->contourLineFramebufferSize[1], 0, GL_LUMINANCE,
      GL_UNSIGNED_BYTE, 0);
    glBindTexture(GL_TEXTURE_RECTANGLE_ARB, 0);

    if (mustAttachBuffers) {
      glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
        GL_RENDERBUFFER_EXT, dataItem->contourLineDepthBufferObject);
      glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
        GL_TEXTURE_RECTANGLE_ARB, dataItem->contourLineColorTextureObject, 0);
  ...
  /* Shift the projection matrix by half a pixel to render the corners of the
   * final pixels: */
...
SurfaceRenderer::SurfaceRenderer(const DepthImageRenderer *sDepthImageRenderer)
    : depthImageRenderer(sDepthImageRenderer), drawContourLines(true),
      contourLineFactor(1.0f), elevationColorMap(0), drawDippingBed(false),
      dippingBedFolded(false), dippingBedPlane(Plane::Vector(0, 0, 1), 0.0f),
      dippingBedThickness(1), dem(0), demDistScale(1.0f), illuminate(false),
      waterTable(0), advectWaterTexture(false), waterOpacity(2.0f),
      surfaceSettingsVersion(1), animationTime(0.0) {
  ...
  fileMonitor.addPath(
    (std::string(CONFIG_SHADERDIR) + std::string("/SurfaceAddContourLines.fs"))
      .c_str(),
    IO::FileMonitor::Modified,
    Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
  fileMonitor.addPath(
    (std::string(CONFIG_SHADERDIR) + std::string("/SurfaceIlluminate.fs"))
      .c_str(),
    IO::FileMonitor::Modified,
    Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
  fileMonitor.addPath(
    (std::string(CONFIG_SHADERDIR) + std::string("/SurfaceAddWaterColor.fs"))
      .c_str(),
    IO::FileMonitor::Modified,
    Misc::createFunctionCall(this, &SurfaceRenderer::shaderSourceFileChanged));
...
void SurfaceRenderer::initContext(GLContextData &contextData) const {
  ...
  dataItem->heightMapShader = createSinglePassSurfaceShader(
    *contextData.getLightTracker(), dataItem->heightMapShaderUniforms,
    dataItem->depthMaskUniform);
  ...
  dataItem->globalAmbientHeightMapShader = linkVertexAndFragmentShader(
    "SurfaceGlobalAmbientHeightMapShader");
  ...
  dataItem->shadowedIlluminatedHeightMapShader = linkVertexAndFragmentShader(
    "SurfaceShadowedIlluminatedHeightMapShader");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[0] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "depthSampler");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[1] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "depthProjection");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[2] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "tangentDepthProjection");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[3] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "basePlane");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[4] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader,
    "pixelCornerElevationSampler");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[5] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "contourLineFactor");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[6] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "heightColorMapSampler");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[7] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader,
    "heightColorMapTransformation");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[8] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "waterLevelSampler");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[9] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader,
    "waterLevelTextureTransformation");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[10] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "waterOpacity");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[11] = glGetUniformLocationARB(
    dataItem->shadowedIlluminatedHeightMapShader, "shadowTextureSampler");
  dataItem
    ->shadowedIlluminatedHeightMapShaderUniforms[12] = glGetUniformLocationARB(
...
void SurfaceRenderer::setElevationColorMap(
...
void SurfaceRenderer::setDippingBedPlane(
  const SurfaceRenderer::Plane &newDippingBedPlane) {
...
void SurfaceRenderer::setDippingBedCoeffs(
  const GLfloat newDippingBedCoeffs[5]) {
...
void SurfaceRenderer::renderSinglePass(const int viewport[4],
  ...
  if (drawContourLines) {
    /* Run the first rendering pass to create a half-pixel offset texture of
     * surface elevations: */
    renderPixelCornerElevations(viewport, projectionModelview, contextData,
      dataItem);
    ...
    glDeleteRenderbuffersEXT(0, &dataItem->contourLineDepthBufferObject);
  ...
  if (dataItem->surfaceSettingsVersion != surfaceSettingsVersion ||
    (illuminate &&
      dataItem->lightTrackerVersion !=
        contextData.getLightTracker()->getVersion())) {
    /* Rebuild the shader: */
    try {
      GLhandleARB newShader = createSinglePassSurfaceShader(
        *contextData.getLightTracker(), dataItem->heightMapShaderUniforms,
        dataItem->depthMaskUniform);
      glDeleteObjectARB(dataItem->heightMapShader);
      dataItem->heightMapShader = newShader;
    } catch (const std::runtime_error &err) {
      Misc::formattedUserError("SurfaceRenderer::renderSinglePass: Caught "
                               "exception %s while rebuilding surface shader",
        err.what());
    }
  ...
  if constexpr (RenderDepthMask) {
    ...
    glUniform2fARB(dataItem->depthMaskUniform[1],
  ...
  if (illuminate) {
    ...
    const Scalar *tmdpPtr = tangentModelviewDepthProjection.getMatrix()
                              .getEntries();
    GLfloat matrix[16];
    GLfloat *mPtr = matrix;
  ...
  projectionModelviewDepthProjection *= depthImageRenderer
                                          ->getDepthProjection();
  ...
  if constexpr (RenderDepthMask) {
    ...
    // We need to restore the original buffer, otherwise Vrui rendering breaks.
    glActiveTextureARB(GL_TEXTURE0_ARB);
  }
