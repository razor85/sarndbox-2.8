...
template <>
class BlobProperty<unsigned short> // Class to calculate the 3D centroid of a
                                   // blob in depth image space
{
  /* Elements: */
private:
...
public:
  BlobProperty(void) : pxs(0.0), pys(0.0), pzs(0.0), numPixels(0) {}
  ...
  size_t getNumPixels(void) const { return numPixels; }
  Geometry::Point<double, 3> calcCentroid(
    void) const // Returns the centroid of the blob in depth image space
  {
    return Geometry::Point<double, 3>(pxs / double(numPixels),
      pys / double(numPixels), pzs / double(numPixels));
  }
};

template <>
class BlobProperty<float> // Class to calculate the 3D centroid of a blob in
                          // depth image space
{
  /* Elements: */
private:
...
class ValidPixelProperty // Functor class to identify valid pixels in raw depth
                         // frames
{
  /* Elements: */
private:
  float minPlane[4]; // Plane equation of the lower bound of valid depth values
                     // in depth image space
  float maxPlane[4]; // Plane equation of the upper bound of valid depth values
                     // in depth image space
  Geometry::Matrix<float, 3, 4>
    colorDepthHomography; // Homography from 3D depth image space into 2D color
                          // image space
...
public:
  ValidPixelProperty(const float sMinPlane[4], const float sMaxPlane[4],
    const Geometry::Matrix<float, 3, 4> &sColorDepthHomography,
    const unsigned int sColorSize[2])
      : colorDepthHomography(sColorDepthHomography), colorFrame(0) {
...
public:
  ...
  bool operator()(unsigned int x, unsigned int y,
    const unsigned short &pixel) const {
  ...
  bool operator()(unsigned int x, unsigned int y, const float &pixel) const {
    ...
    float pz = pixel;
    float minD = minPlane[0] * px + minPlane[1] * py + minPlane[2] * pz +
      minPlane[3];
    float maxD = maxPlane[0] * px + maxPlane[1] * py + maxPlane[2] * pz +
      maxPlane[3];
...
inline void RainMaker::extractBlobs(const Kinect::FrameBuffer &depthFrame,
  const ValidPixelProperty &vpp, RainMaker::BlobList &blobsCc) {
  ...
  for (typename std::vector<::Blob<DepthPixelParam>>::const_iterator bIt =
         blobsDic.begin();
       bIt != blobsDic.end(); ++bIt)
    if (bIt->max[0] - bIt->min[0] >= minBlobSize &&
      bIt->max[1] - bIt->min[1] >= minBlobSize) {
      Blob blobCc;
      Point centroidDic = bIt->blobProperty.calcCentroid();
      blobCc.centroid = depthProjection.transform(centroidDic);

      /* Estimate the radius of the blob in camera space (this is admittedly
       * ad-hoc): */
      double radiusDic = double(bIt->max[0] - bIt->min[0]) * 0.5;
      if (radiusDic > (bIt->max[1] - bIt->min[1]) * 0.5) {
        radiusDic = (bIt->max[1] - bIt->min[1]) * 0.5;
        blobCc.radius = Geometry::dist(
          depthProjection.transform(
            Point(centroidDic[0], centroidDic[1] + radiusDic, centroidDic[2])),
          blobCc.centroid);
      } else
        blobCc.radius = Geometry::dist(
          depthProjection.transform(
            Point(centroidDic[0] + radiusDic, centroidDic[1], centroidDic[2])),
          blobCc.centroid);
...
void *RainMaker::detectionThreadMethod(void) {
  ...
  while (true) {
    Kinect::FrameBuffer depthFrame, colorFrame;
    {
      ...
      /* Wait until a new depth and color frame arrive, or the program shuts
       * down: */
      while (runDetectionThread &&
...
RainMaker::RainMaker(const unsigned int sDepthSize[2],
  const unsigned int sColorSize[2],
  const RainMaker::PTransform &sDepthProjection,
  const RainMaker::PTransform &sColorProjection,
  const RainMaker::Plane &basePlane, double minElevation, double maxElevation,
  int sMinBlobSize)
    : depthIsFloat(false), outputBlobsFunction(0) {
  ...
  /* Calculate the direct homography from depth image space to color image
   * space: */
  PTransform hom = PTransform::scale(PTransform::Scale(double(colorSize[0]),
    double(colorSize[1]), 1.0)); // Go to color image space
  ...
  /* Calculate the equations of the minimum and maximum elevation planes in
   * camera space: */
  PTransform::HVector minPlaneCc(basePlane.getNormal());
  minPlaneCc[3] = -(
    basePlane.getOffset() + minElevation * basePlane.getNormal().mag());
  PTransform::HVector maxPlaneCc(basePlane.getNormal());
  maxPlaneCc[3] = -(
    basePlane.getOffset() + maxElevation * basePlane.getNormal().mag());

  /* Transform the plane equations to depth image space and flip and swap the
   * min and max planes because elevation increases opposite to raw depth: */
  PTransform::HVector minPlaneDic(
    depthProjection.getMatrix().transposeMultiply(minPlaneCc));
  double minPlaneScale = -1.0 / Geometry::mag(minPlaneDic.toVector());
  for (int i = 0; i < 4; ++i)
    maxPlane[i] = float(minPlaneDic[i] * minPlaneScale);
  PTransform::HVector maxPlaneDic(
    depthProjection.getMatrix().transposeMultiply(maxPlaneCc));
...
void RainMaker::setOutputBlobsFunction(
  RainMaker::OutputBlobsFunction *newOutputBlobsFunction) {
  delete outputBlobsFunction;
