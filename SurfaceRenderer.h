...
private:
  struct DataItem : public GLObject::DataItem {
    /* Elements: */
  public:
    GLuint contourLineFramebufferSize[2]; // Current width and height of contour
                                          // line rendering frame buffer
    GLuint contourLineFramebufferObject;  // Frame buffer object used to render
                                          // topographic contour lines
    GLuint contourLineDepthBufferObject;  // Depth render buffer for topographic
                                          // contour line frame buffer
    GLuint
      contourLineColorTextureObject; // Color texture object for topographic
                                     // contour line frame buffer
    unsigned int contourLineVersion; // Version number of depth image used for
    ...
    unsigned int
      surfaceSettingsVersion; // Version number of surface settings for which
                              // the height map shader was built
    unsigned int
      lightTrackerVersion; // Version number of light tracker state for which
                           // the height map shader was built
    GLhandleARB
      globalAmbientHeightMapShader; // Shader program to render the global
                                    // ambient component of the surface using a
                                    // height color map
    GLint
      globalAmbientHeightMapShaderUniforms[13]; // Locations of the global
                                                // ambient height map shader's
                                                // uniform variables
    GLhandleARB
      shadowedIlluminatedHeightMapShader; // Shader program to render the
                                          // surface using illumination with
                                          // shadows and a height color map
  ...
  const DepthImageRenderer
    *depthImageRenderer;          // Renderer for low-level surface rendering
  unsigned int depthImageSize[2]; // Size of depth image texture
  PTransform
    tangentDepthProjection; // Transposed depth projection matrix for tangent
                            // planes, i.e., homogeneous normal vectors
  IO::FileMonitor
    fileMonitor; // Monitor to watch the renderer's external shader source files

  bool drawContourLines;     // Flag if topographic contour lines are enabled
  GLfloat contourLineFactor; // Inverse elevation distance between adjacent
                             // topographic contour lines

  ElevationColorMap *elevationColorMap; // Pointer to a color map for
                                        // topographic elevation map coloring
  ...
  DEM *dem; // Pointer to a pre-made digital elevation model to create a
            // zero-surface for height color mapping
  GLfloat
    demDistScale; // Maximum deviation from surface to DEM in camera-space units

  bool illuminate; // Flag whether the surface shall be illuminated

  WaterTable2
    *waterTable; // Pointer to the water table object; if NULL, water is ignored
  bool advectWaterTexture; // Flag whether water texture coordinates are
                           // advected to visualize water flow
  GLfloat waterOpacity;    // Scaling factor for water opacity

  unsigned int
    surfaceSettingsVersion; // Version number of surface settings to invalidate
                            // surface rendering shader on changes
  ...
  void renderPixelCornerElevations(const int viewport[4],
    ...
    DataItem *dataItem) const; // Creates texture containing pixel-corner
                               // elevations based on the current depth image

  /* Constructors and destructors: */
public:
  SurfaceRenderer(const DepthImageRenderer
      *sDepthImageRenderer); // Creates a renderer for the given depth image
                             // renderer
  ...
  /* New methods: */
  void setDrawContourLines(
    bool newDrawContourLines); // Enables or disables topographic contour lines
  void setContourLineDistance(
    GLfloat newContourLineDistance); // Sets the elevation distance between
                                     // adjacent topographic contour lines
  void setElevationColorMap(
    ElevationColorMap *newElevationColorMap);     // Sets an elevation color map
  void setDrawDippingBed(bool newDrawDippingBed); // Sets the dipping bed flag
  void setDippingBedPlane(
    const Plane &newDippingBedPlane); // Sets the dipping bed plane equation
  void setDippingBedCoeffs(const GLfloat
      newDippingBedCoeffs[5]); // Sets folding dipping bed's coefficients
  void setDippingBedThickness(
    GLfloat newDippingBedThickness); // Sets the thickness of the dipping bed in
                                     // camera-space units
  void setDem(DEM *newDem); // Sets a pre-made digital elevation model to create
                            // a zero surface for height color mapping
  void setDemDistScale(
    GLfloat newDemDistScale); // Sets the deviation from DEM to surface to
                              // saturate the deviation color map
  void setIlluminate(bool newIlluminate); // Sets the illumination flag
  void setWaterTable(
    WaterTable2 *newWaterTable); // Sets the pointer to the water table; NULL
                                 // disables water handling
  void setAdvectWaterTexture(
    bool newAdvectWaterTexture); // Sets the water texture coordinate advection
                                 // flag
  void setWaterOpacity(
    GLfloat newWaterOpacity); // Sets the water opacity factor
