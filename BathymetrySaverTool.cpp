...
BathymetrySaverToolFactory::Configuration::Configuration(void)
    : saveFileName("BathymetrySaverTool.dem"), postUpdate(false),
      postUpdatePort(80), postUpdatePage(""),
      postUpdateMessage("app.GenerateTileCache();"), gridScale(1.0) {}

void BathymetrySaverToolFactory::Configuration::read(
  const Misc::ConfigurationFileSection &cfs) {
  saveFileName = cfs.retrieveString("./saveFileName", saveFileName);
  postUpdate = cfs.retrieveValue<bool>("./postUpdate", postUpdate);
  postUpdateHostName = cfs.retrieveString("./postUpdateHostName",
    postUpdateHostName);
  ...
  postUpdateMessage = cfs.retrieveString("./postUpdateMessage",
    postUpdateMessage);
  gridScale = cfs.retrieveValue<double>("./gridScale", gridScale);
}

void BathymetrySaverToolFactory::Configuration::write(
  Misc::ConfigurationFileSection &cfs) const {
...
BathymetrySaverToolFactory::BathymetrySaverToolFactory(WaterTable2 *sWaterTable,
  Vrui::ToolManager &toolManager)
    : ToolFactory("BathymetrySaverTool", toolManager), waterTable(sWaterTable) {
  ...
  /* Load class settings: */
  Misc::ConfigurationFileSection cfs = toolManager.getToolClassSection(
    getClassName());
...
Vrui::Tool *BathymetrySaverToolFactory::createTool(
  const Vrui::ToolInputAssignment &inputAssignment) const {
...
std::ostream &printFloat4(std::ostream &os, double value) {
  if (value != 0.0) {
    ...
    std::ios::fmtflags oldFlags = os.flags(
      std::ios::showpoint | std::ios::dec | std::ios::fixed | std::ios::right);
...
void BathymetrySaverTool::writeDEMFile(void) const {
  ...
  IO::OStream demFile(
    IO::openFile(configuration.saveFileName.c_str(), IO::File::WriteOnly));
  ...
  double west = gridCenter[0] -
    double(factory->gridSize[0] - 1) * double(factory->cellSize[0]) * gs * 0.5;
  double east = gridCenter[0] +
    double(factory->gridSize[0] - 1) * double(factory->cellSize[0]) * gs * 0.5;
  double north = gridCenter[1] +
    double(factory->gridSize[1] - 1) * double(factory->cellSize[1]) * gs * 0.5;
  double south = gridCenter[1] -
    double(factory->gridSize[1] - 1) * double(factory->cellSize[1]) * gs * 0.5;
  ...
  for (GLsizei count = factory->gridSize[1] * factory->gridSize[0] - 1;
       count > 0; --count, ++bbPtr) {
    if (elevMin > *bbPtr)
      elevMin = *bbPtr;
    if (elevMax < *bbPtr)
  ...
  if (elevRange != 0.0) {
    /* Calculate a power-of-ten scale factor to scale the actual terrain range
     * to -9999 to 9999: */
  ...
  // std::cout<<(elevMax-elevationBase)*zScale<<",
  // "<<(elevMin-elevationBase)*zScale<<std::endl;
  ...
  /* Write the grid scales with full accuracy. Per spec, only integer values are
   * supported: */
  ...
  for (GLsizei column = 0; column < factory->gridSize[0]; ++column) {
    ...
    printFloat8(demFile,
      west +
        double(column) * double(factory->cellSize[0]) *
          gs); // Easting of first elevation posting in column
    printFloat8(demFile,
      south); // Northing of first elevation posting in column
    ...
    for (GLsizei count = factory->gridSize[1] - 1; count > 0;
         --count, pPtr += factory->gridSize[0]) {
      if (elevMin > *pPtr)
        elevMin = *pPtr;
      if (elevMax < *pPtr)
        elevMax = *pPtr;
    ...
    for (GLsizei count = factory->gridSize[1]; count > 0;
         --count, pPtr += factory->gridSize[0]) {
      /* Check if there is enough space left in the current 1024-character
       * record: */
      size_t paddedSize = (fileSize + 1023U) & ~size_t(1023U);
...
void BathymetrySaverTool::postUpdate(void) const {
  ...
  Comm::NetPipePtr pipe = new Comm::TCPPipe(
    configuration.postUpdateHostName.c_str(), configuration.postUpdatePort);
  ...
  request.append(Misc::print(configuration.postUpdateMessage.size(),
    contentLengthString + 5));
  request.append("\r\n");
  ...
  size_t replySize = 0;
  {
    ...
    if (statusCode != 200)
      Misc::throwStdErr("HTTP error %d: %s", statusCode,
        reply.readLine().c_str());
    reply.readLine();
    reply.skipWs();
...
void BathymetrySaverTool::readBackCallback(GLfloat *bathymetryBuffer,
  GLfloat *waterLevelBuffer, void *userData) {
  ...
  try {
    ...
    Misc::formattedUserError(
      "Save Bathymetry: Unable to save bathymetry due to exception \"%s\"",
      err.what());
  }
}

BathymetrySaverToolFactory *BathymetrySaverTool::initClass(
  WaterTable2 *sWaterTable, Vrui::ToolManager &toolManager) {
  /* Create the tool factory: */
  factory = new BathymetrySaverToolFactory(sWaterTable, toolManager);

  /* Register and return the class: */
  toolManager.addClass(factory,
...
BathymetrySaverTool::~BathymetrySaverTool(void) { delete[] bathymetryBuffer; }

void BathymetrySaverTool::configure(
  const Misc::ConfigurationFileSection &configFileSection) {
  /* Override private configuration data from given configuration file section:
   */
  configuration.read(configFileSection);
}

void BathymetrySaverTool::storeState(
  Misc::ConfigurationFileSection &configFileSection) const {
...
void BathymetrySaverTool::buttonCallback(int buttonSlotIndex,
  ...
  if (cbData->newButtonState) {
    ...
    application->gridRequest.requestGrids(bathymetryBuffer, 0,
      &BathymetrySaverTool::readBackCallback, this);
  }
}
