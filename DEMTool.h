...
private:
  GLMotif::FileSelectionHelper
    demSelectionHelper; // Helper object to load DEMs from files
...
class DEMTool : public DEM,
  ...
  /* Elements: */
private:
  static DEMToolFactory
    *factory;               // Pointer to the factory object for this class
  std::string demFileName;  // Name of DEM file to load
  bool haveDemTransform;    // Flag if the tool's configuration file section
                            // specified a DEM transformation
  OGTransform demTransform; // The transformation to apply to the DEM
  Scalar demVerticalShift;  // Extra vertical shift to apply to DEM in sandbox
                            // coordinate units
  ...
  void loadDEMFileCallback(GLMotif::FileSelectionDialog::OKCallbackData
      *cbData); // Called when the user selects a DEM file to load

  /* Constructors and destructors: */
public:
  static DEMToolFactory *initClass(Vrui::ToolManager &toolManager);
  DEMTool(const Vrui::ToolFactory *factory,
