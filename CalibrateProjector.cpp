...
CalibrateProjector::CaptureToolFactory
  *CalibrateProjector::CaptureTool::factory = 0;
...
const Vrui::ToolFactory *CalibrateProjector::CaptureTool::getFactory(
  void) const {
  return factory;
}
...
void CalibrateProjector::depthStreamingCallback(
...
void CalibrateProjector::meshStreamingCallback(
  const Kinect::MeshBuffer &meshBuffer) {
  /* Update application state: */
  Vrui::requestUpdate();
}
...
void CalibrateProjector::backgroundCaptureCompleteCallback(
...
void CalibrateProjector::diskExtractionCallback(
  const Kinect::DiskExtractor::DiskList &disks) {
...
CalibrateProjector::CalibrateProjector(int &argc, char **&argv)
    : Vrui::Application(argc, argv), numTiePointFrames(60),
      numBackgroundFrames(120), camera(0), diskExtractor(0), projector(0),
      capturingBackground(false), capturingTiePoint(false), numCaptureFrames(0),
      tiePointIndex(0), haveProjection(false), projection(4, 4) {
  /* Register the custom tool class: */
  CaptureToolFactory *toolFactory1 = new CaptureToolFactory("CaptureTool",
    "Capture", 0, *Vrui::getToolManager());
  ...
  Vrui::getToolManager()->addClass(toolFactory1,
    ...
    if (argv[i][0] == '-') {
      if (strcasecmp(argv[i] + 1, "h") == 0)
        ...
        if (i < argc) {
          ...
          remoteSource = Kinect::MultiplexedFrameSource::create(
            Comm::openTCPPipe(argv[i - 1], atoi(argv[i])));
        }
  ...
  if (printHelp) {
    std::cout << "Usage: CalibrateProjector [option 1] ... [option n]"
              << std::endl;
    ...
    std::cout
      << "     Connects to a remote 3D video server on the given host name /"
      << std::endl;
    std::cout << "     port number" << std::endl;
    std::cout << "     Default: <empty>" << std::endl;
    std::cout << "  -c <camera index>" << std::endl;
    std::cout
      << "     Selects the 3D camera of the given index on the local USB bus or"
      << std::endl;
    std::cout << "     on the remote 3D video server (0: first camera)"
              << std::endl;
    std::cout << "     Default: 0" << std::endl;
    std::cout << "  -s <projector image width> <projector image height>"
              << std::endl;
    std::cout
      << "     Sets the width and height of the projector image in pixels. This"
      << std::endl;
    std::cout << "     must match the actual resolution of the projector."
              << std::endl;
    std::cout << "     Default: 1024 768" << std::endl;
    std::cout << "  -tp <grid width> <grid height>" << std::endl;
    std::cout << "     Sets the number of tie points to be collected before a "
                 "calibration"
              << std::endl;
    ...
    std::cout
      << "     Maximum depth distance between adjacent pixels in the same blob."
      << std::endl;
    std::cout << "     Default: 1" << std::endl;
    std::cout << "  -tpf <tie point file name>" << std::endl;
    std::cout << "     Reads initial calibration tie points from a CSV file"
              << std::endl;
    std::cout << "  -pmf <projection matrix file name>" << std::endl;
    std::cout
      << "     Saves the calibration matrix to the file of the given name"
      << std::endl;
    std::cout << "     Default: " << CONFIG_CONFIGDIR << '/'
  ...
  /* Read the sandbox layout file: */
  {
    ...
    basePlane = Misc::ValueCoder<OPlane>::decode(s.c_str(),
      s.c_str() + s.length());
    basePlane.normalize();
    for (int i = 0; i < 4; ++i) {
      layoutSource.skipWs();
      ...
      basePlaneCorners[i] = basePlane.project(
        Misc::ValueCoder<OPoint>::decode(s.c_str(), s.c_str() + s.length()));
    }
  }
  ...
  /* Calculate the transformation from camera space to sandbox space: */
  {
    ...
    boxTransform *= ONTransform::translateToOriginFrom(
      basePlane.project(center));
  }
  ...
  if (remoteSource != 0) {
    ...
    Kinect::DirectFrameSource *directCamera = Kinect::openDirectFrameSource(
      cameraIndex, false);
    camera = directCamera;
  ...
  diskExtractor = new Kinect::DiskExtractor(
    camera->getActualFrameSize(Kinect::FrameSource::DEPTH),
  ...
  projector->setExtrinsicParameters(
    Kinect::FrameSource::ExtrinsicParameters::identity);
...
#if !KINECT_CONFIG_USE_SHADERPROJECTOR
  projector->startStreaming(
    Misc::createFunctionCall(this, &CalibrateProjector::meshStreamingCallback));
#endif
  camera->startStreaming(
    Misc::createFunctionCall(projector, &Kinect::ProjectorType::setColorFrame),
...
void CalibrateProjector::frame(void) {
  /* Check if we are capturing a tie point and there is a new list of extracted
   * disks: */
  if (diskList.lockNewValue() && capturingTiePoint &&
    diskList.getLockedValue().size() == 1) {
...
void CalibrateProjector::display(GLContextData &contextData) const {
  ...
  if (capturingBackground) {
    ...
    if (bbw * sh >= sw * bbh) // Sandbox area is wider
    {
      ...
      glOrtho(bbox.min[0], bbox.max[0], bbox.min[1] - filler,
        bbox.max[1] + filler, -200.0, 200.0);
    } else // Sandbox area is taller
    {
      Scalar filler = Math::div2((bbh * sw) / sh - bbw);
      glOrtho(bbox.min[0] - filler, bbox.max[0] + filler, bbox.min[0],
        bbox.max[0], -200.0, 200.0);
    }
    ...
    for (Kinect::DiskExtractor::DiskList::const_iterator dlIt = dl.begin();
         dlIt != dl.end(); ++dlIt) {
      glPushMatrix();
      glTranslate(dlIt->center - Kinect::DiskExtractor::Point::origin);
      glRotate(Vrui::Rotation::rotateFromTo(Vrui::Vector(0, 0, 1),
        Vrui::Vector(dlIt->normal)));

      glBegin(GL_POLYGON);
      glColor3f(0.0f, 1.0f, 0.0f);
      for (int i = 0; i < 64; ++i) {
        Vrui::Scalar angle = Vrui::Scalar(i) * Vrui::Scalar(2) *
          Math::Constants<Vrui::Scalar>::pi / Vrui::Scalar(64);
        glVertex3d(Math::cos(angle) * dlIt->radius,
          Math::sin(angle) * dlIt->radius, 0.0);
      }
      glEnd();
    ...
    if (haveProjection) {
      ...
      for (Kinect::DiskExtractor::DiskList::const_iterator dlIt = dl.begin();
        ...
        double x = (projBlob(0) / projBlob(3) + 1.0) * double(imageSize[0]) /
          2.0;
        double y = (projBlob(1) / projBlob(3) + 1.0) * double(imageSize[1]) /
          2.0;
        glBegin(GL_LINES);
        glColor3f(1.0f, 0.0f, 0.0f);
        glVertex2d(x, 0.0);
...
void CalibrateProjector::startBackgroundCapture(void) {
  ...
  Kinect::DirectFrameSource
    *directCamera = dynamic_cast<Kinect::DirectFrameSource *>(camera);
  if (directCamera != 0) {
    /* Tell the 3D camera to capture a new background frame: */
    capturingBackground = true;
    std::cout << "CalibrateProjector: Capturing " << numBackgroundFrames
              << " background frames..." << std::flush;
    directCamera->captureBackground(numBackgroundFrames, true,
      Misc::createFunctionCall(this,
...
void CalibrateProjector::startTiePointCapture(void) {
  ...
  std::cout << "CalibrateProjector: Capturing " << numTiePointFrames
            << " tie point frames..." << std::flush;
}
...
void CalibrateProjector::calcCalibration(void) {
  ...
  for (std::vector<TiePoint>::iterator tpIt = tiePoints.begin();
       tpIt != tiePoints.end(); ++tpIt) {
    // DEBUGGING
    // std::cout<<"Tie point: "<<tpIt->p[0]<<", "<<tpIt->p[1]<<",
    // "<<tpIt->o[0]<<", "<<tpIt->o[1]<<", "<<tpIt->o[2]<<std::endl;
  ...
  /* Scale the homography such that projected weights are positive distance from
   * projector: */
  double wLen = Math::sqrt(
    Math::sqr(hom(2, 0)) + Math::sqr(hom(2, 1)) + Math::sqr(hom(2, 2)));
  ...
  if (numNegativeWeights == 0 || numNegativeWeights == int(tiePoints.size())) {
    ...
    std::cout << "Z range of collected tie points: [" << zRange.getMin() << ", "
              << zRange.getMax() << "]" << std::endl;

    /* Double the size of the range to include a safety margin on either side:
     */
    zRange = Math::Interval<double>(zRange.getMin() * 2.0,
      zRange.getMax() * 0.5);

    /* Pre-multiply the projection matrix with the inverse viewport matrix to go
     * to clip coordinates: */
    ...
    IO::FilePtr projFile = IO::openFile(projectionMatrixFileName.c_str(),
      IO::File::WriteOnly);
    ...
    std::cout << "Calibration error: Some tie points have negative projection "
                 "weights. Please start from scratch"
              << std::endl;
}
