...
GlobalWaterToolFactory *GlobalWaterTool::initClass(
  Vrui::ToolManager &toolManager) {
  /* Create the tool factory: */
  factory = new GlobalWaterToolFactory("GlobalWaterTool", "Manage Water", 0,
    toolManager);
...
GlobalWaterTool::GlobalWaterTool(const Vrui::ToolFactory *factory,
...
void GlobalWaterTool::buttonCallback(int buttonSlotIndex,
  ...
  if (cbData->newButtonState) // Button was just pressed
  {
    ...
    waterAmount = application->waterSpeed > 0.0 ?
      application->rainStrength / application->waterSpeed :
      0.0f;
  ...
  application->waterTable->setWaterDeposit(
    application->waterTable->getWaterDeposit() + waterAmount);
}
