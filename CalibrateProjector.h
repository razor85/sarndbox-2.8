...
private:
  ...
  typedef Kinect::DiskExtractor::Point
    OPoint; // Type for 3D points in object (camera) space
  typedef Kinect::DiskExtractor::Vector Vector; // Type for 3D vectors
  typedef Geometry::Point<Scalar, 2>
    PPoint; // Type for 2D points in projection space
  ...
  class CaptureTool : public Vrui::Tool,
                      public Vrui::Application::Tool<CalibrateProjector> {
    friend class Vrui::GenericToolFactory<CaptureTool>;

    /* Elements: */
  private:
    static CaptureToolFactory
      *factory; // Pointer to the factory object for this class

    /* Constructors and destructors: */
  public:
    CaptureTool(const Vrui::ToolFactory *factory,
...
private:
  ...
  ONTransform boxTransform; // Transformation from camera space to sandbox space
  ...
  unsigned int
    numBackgroundFrames; // Number of frames to capture for background removal

  Kinect::FrameSource *camera; // 3D video source to calibrate
  Kinect::DiskExtractor
    *diskExtractor; // Object to extract disk shapes from a 3D video stream
  Kinect::ProjectorType *projector; // A projector to render the 3D video stream
  bool capturingBackground; // Flag if the 3D camera is currently capturing a
                            // background frame
  bool capturingTiePoint; // Flag whether the main thread is currently capturing
                          // a tie point
  unsigned int numCaptureFrames; // Number of background or tie point frames
                                 // still to capture

  Threads::TripleBuffer<Kinect::DiskExtractor::DiskList>
    diskList;                      // Triple buffer of lists of extracted disks
  ...
  std::string projectionMatrixFileName; // Name of the file to which the
                                        // projection matrix is saved

  /* Private methods: */
  void depthStreamingCallback(const Kinect::FrameBuffer
      &frameBuffer); // Callback receiving depth frames from the 3D camera
#if !KINECT_CONFIG_USE_SHADERPROJECTOR
  void meshStreamingCallback(
    const Kinect::MeshBuffer &meshBuffer); // Callback receiving projected
                                           // meshes from the 3D video projector
#endif
  void backgroundCaptureCompleteCallback(
    Kinect::DirectFrameSource &camera); // Callback when the 3D camera is done
                                        // capturing a background image
  void diskExtractionCallback(const Kinect::DiskExtractor::DiskList
      &disks); // Called when a new list of disks has been extracted
...
public:
  ...
  void calcCalibration(void); // Calculates the calibration transformation after
                              // all tie points have been collected
};
