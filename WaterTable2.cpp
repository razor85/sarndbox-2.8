...
WaterTable2::DataItem::DataItem(void)
    : currentBathymetry(0), bathymetryVersion(0), currentQuantity(0),
      ...
      bathymetryFramebufferObject(0), derivativeFramebufferObject(0),
      maxStepSizeFramebufferObject(0), integrationFramebufferObject(0),
      waterFramebufferObject(0), bathymetryShader(0), waterAdaptShader(0),
      derivativeShader(0), maxStepSizeShader(0), boundaryShader(0),
      eulerStepShader(0), rungeKuttaStepShader(0), waterAddShader(0),
      waterShader(0) {
  for (int i = 0; i < 2; ++i) {
...
void WaterTable2::calcTransformations(void) {
  /* Calculate the combined modelview and projection matrix to render depth
   * images into the bathymetry grid: */
  {
  ...
  /* Calculate the combined modelview and projection matrix to render
   * water-adding geometry into the water texture: */
  {
...
GLfloat WaterTable2::calcDerivative(WaterTable2::DataItem *dataItem,
  GLuint quantityTextureObject, bool calcMaxStepSize) const {
  /*********************************************************************
  ...
  *********************************************************************/
  ...
  glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,
    dataItem->derivativeFramebufferObject);
  ...
  *********************************************************************/
  ...
  if (calcMaxStepSize) {
    ...
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,
      dataItem->maxStepSizeFramebufferObject);

    /* Reduce the maximum step size texture in a sequence of half-reduction
     * steps: */
    ...
    while (reducedWidth > 1 || reducedHeight > 1) {
      ...
      glUniformARB(dataItem->maxStepSizeShaderUniformLocations[0],
        GLfloat(reducedWidth - 1), GLfloat(reducedHeight - 1));

      /* Bind the current max step size texture: */
      glActiveTextureARB(GL_TEXTURE0_ARB);
      glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
        dataItem->maxStepSizeTextureObjects[currentMaxStepSizeTexture]);
...
WaterTable2::WaterTable2(GLsizei width, GLsizei height,
  const GLfloat sCellSize[2])
    : depthImageRenderer(0), baseTransform(ONTransform::identity),
      dryBoundary(true) {
...
WaterTable2::WaterTable2(GLsizei width, GLsizei height,
  const DepthImageRenderer *sDepthImageRenderer,
  const Point basePlaneCorners[4])
    : depthImageRenderer(sDepthImageRenderer), dryBoundary(true) {
  ...
  /* Calculate the transformation from camera space to upright elevation model
   * space: */
  ...
  baseTransform *= ONTransform::rotate(
    ONTransform::Rotation::fromBaseVectors(x, y));
...
void WaterTable2::initContext(GLContextData &contextData) const {
  ...
  {
    /* Create the vertex-centered bathymetry textures, replacing the outermost
     * layer of cells with ghost cells: */
    glGenTextures(2, dataItem->bathymetryTextureObjects);
    GLfloat *b = makeBuffer(size[0] - 1, size[1] - 1, 1, double(domain.min[2]));
    for (int i = 0; i < 2; ++i) {
      glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
        dataItem->bathymetryTextureObjects[i]);
      ...
      glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F, size[0] - 1,
        size[1] - 1, 0, GL_LUMINANCE, GL_FLOAT, b);
    }
    delete[] b;
  }

  {
    ...
    GLfloat *q = makeBuffer(size[0], size[1], 3, double(domain.min[2]), 0.0,
      0.0);
    for (int i = 0; i < 3; ++i) {
      glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
        dataItem->quantityTextureObjects[i]);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MIN_FILTER,
        GL_NEAREST);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MAG_FILTER,
        GL_NEAREST);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S, GL_CLAMP);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T, GL_CLAMP);
      glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGB32F, size[0], size[1], 0,
        GL_RGB, GL_FLOAT, q);
    }
    delete[] q;
  }
...

  {
    ...
    for (int i = 0; i < 2; ++i) {
      glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
        dataItem->maxStepSizeTextureObjects[i]);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MIN_FILTER,
        GL_NEAREST);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MAG_FILTER,
        GL_NEAREST);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S, GL_CLAMP);
      glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T, GL_CLAMP);
      glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F, size[0], size[1], 0,
        GL_LUMINANCE, GL_FLOAT, mss);
    }
    delete[] mss;
  }
...

  {
    ...
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,
      dataItem->bathymetryFramebufferObject);
    ...
    for (int i = 0; i < 2; ++i)
      glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
        ...
        dataItem->bathymetryTextureObjects[i], 0);
...

  {
    ...
    /* Attach the derivative and maximum step size textures to the temporal
     * derivative computation frame buffer: */
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
      GL_TEXTURE_RECTANGLE_ARB, dataItem->derivativeTextureObject, 0);
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT1_EXT,
      GL_TEXTURE_RECTANGLE_ARB, dataItem->maxStepSizeTextureObjects[0], 0);
    GLenum drawBuffers[2] = { GL_COLOR_ATTACHMENT0_EXT,
...

  {
    ...
    /* Attach the maximum step size textures to the maximum step size
     * computation frame buffer: */
    for (int i = 0; i < 2; ++i)
      glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
        GL_COLOR_ATTACHMENT0_EXT + i, GL_TEXTURE_RECTANGLE_ARB,
        dataItem->maxStepSizeTextureObjects[i], 0);
    glDrawBuffer(GL_NONE);
    glReadBuffer(GL_NONE);
  }

  {
    ...
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,
      dataItem->integrationFramebufferObject);

    /* Attach the quantity textures to the integration step frame buffer: */
    for (int i = 0; i < 3; ++i)
      glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
        GL_COLOR_ATTACHMENT0_EXT + i, GL_TEXTURE_RECTANGLE_ARB,
        dataItem->quantityTextureObjects[i], 0);
    glDrawBuffer(GL_NONE);
    glReadBuffer(GL_NONE);
  }

  {
    ...
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
      GL_TEXTURE_RECTANGLE_ARB, dataItem->waterTextureObject, 0);
  ...
  static const char *vertexShaderSourceTemplate =
    "void "
    "main(){gl_Position=vec4(gl_Vertex.x*%f-1.0,gl_Vertex.y*%f-1.0,0.0,1.0);}";
  char vertexShaderSource[256];
  snprintf(vertexShaderSource, sizeof(vertexShaderSource),
    vertexShaderSourceTemplate, 2.0 / double(size[0]), 2.0 / double(size[1]));

  /* Create the bathymetry update shader: */
  {
    GLhandleARB vertexShader = glCompileVertexShaderFromString(
      vertexShaderSource);
    GLhandleARB fragmentShader = compileFragmentShader(
      "Water2BathymetryUpdateShader");
    dataItem->bathymetryShader = glLinkShader(vertexShader, fragmentShader);
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->bathymetryShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->bathymetryShader, "oldBathymetrySampler");
    dataItem->bathymetryShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->bathymetryShader, "newBathymetrySampler");
    dataItem->bathymetryShaderUniformLocations[2] = glGetUniformLocationARB(
      dataItem->bathymetryShader, "quantitySampler");
  }
  ...
  /* Create the water adaptation shader: */
  {
    GLhandleARB vertexShader = glCompileVertexShaderFromString(
      vertexShaderSource);
    GLhandleARB fragmentShader = compileFragmentShader(
      "Water2WaterAdaptShader");
    dataItem->waterAdaptShader = glLinkShader(vertexShader, fragmentShader);
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->waterAdaptShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->waterAdaptShader, "bathymetrySampler");
    dataItem->waterAdaptShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->waterAdaptShader, "newQuantitySampler");
  }
  ...
  /* Create the temporal derivative computation shader: */
  {
    GLhandleARB vertexShader = glCompileVertexShaderFromString(
      vertexShaderSource);
    GLhandleARB fragmentShader = compileFragmentShader(
      "Water2SlopeAndFluxAndDerivativeShader");
    dataItem->derivativeShader = glLinkShader(vertexShader, fragmentShader);
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->derivativeShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->derivativeShader, "cellSize");
    dataItem->derivativeShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->derivativeShader, "theta");
    dataItem->derivativeShaderUniformLocations[2] = glGetUniformLocationARB(
      dataItem->derivativeShader, "g");
    dataItem->derivativeShaderUniformLocations[3] = glGetUniformLocationARB(
      dataItem->derivativeShader, "epsilon");
    dataItem->derivativeShaderUniformLocations[4] = glGetUniformLocationARB(
      dataItem->derivativeShader, "bathymetrySampler");
    dataItem->derivativeShaderUniformLocations[5] = glGetUniformLocationARB(
      dataItem->derivativeShader, "quantitySampler");
  }
  ...
  /* Create the maximum step size gathering shader: */
  {
    GLhandleARB vertexShader = glCompileVertexShaderFromString(
      vertexShaderSource);
    GLhandleARB fragmentShader = compileFragmentShader(
      "Water2MaxStepSizeShader");
    dataItem->maxStepSizeShader = glLinkShader(vertexShader, fragmentShader);
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->maxStepSizeShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->maxStepSizeShader, "fullTextureSize");
    dataItem->maxStepSizeShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->maxStepSizeShader, "maxStepSizeSampler");
  }
  ...
  /* Create the boundary condition shader: */
  {
    ...
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->boundaryShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->boundaryShader, "bathymetrySampler");
  }
  ...
  /* Create the Euler integration step shader: */
  {
    ...
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->eulerStepShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->eulerStepShader, "stepSize");
    dataItem->eulerStepShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->eulerStepShader, "attenuation");
    dataItem->eulerStepShaderUniformLocations[2] = glGetUniformLocationARB(
      dataItem->eulerStepShader, "quantitySampler");
    dataItem->eulerStepShaderUniformLocations[3] = glGetUniformLocationARB(
      dataItem->eulerStepShader, "derivativeSampler");
  }
  ...
  /* Create the Runge-Kutta integration step shader: */
  {
    GLhandleARB vertexShader = glCompileVertexShaderFromString(
      vertexShaderSource);
    GLhandleARB fragmentShader = compileFragmentShader(
      "Water2RungeKuttaStepShader");
    dataItem->rungeKuttaStepShader = glLinkShader(vertexShader, fragmentShader);
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->rungeKuttaStepShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->rungeKuttaStepShader, "stepSize");
    dataItem->rungeKuttaStepShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->rungeKuttaStepShader, "attenuation");
    dataItem->rungeKuttaStepShaderUniformLocations[2] = glGetUniformLocationARB(
      dataItem->rungeKuttaStepShader, "quantitySampler");
    dataItem->rungeKuttaStepShaderUniformLocations[3] = glGetUniformLocationARB(
      dataItem->rungeKuttaStepShader, "quantityStarSampler");
    dataItem->rungeKuttaStepShaderUniformLocations[4] = glGetUniformLocationARB(
      dataItem->rungeKuttaStepShader, "derivativeSampler");
  }
  ...
  /* Create the water adder rendering shader: */
  {
    ...
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->waterAddShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->waterAddShader, "pmv");
    dataItem->waterAddShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->waterAddShader, "stepSize");
    dataItem->waterAddShaderUniformLocations[2] = glGetUniformLocationARB(
      dataItem->waterAddShader, "waterSampler");
  }
  ...
  /* Create the water shader: */
  {
    GLhandleARB vertexShader = glCompileVertexShaderFromString(
      vertexShaderSource);
    GLhandleARB fragmentShader = compileFragmentShader(
      "Water2WaterUpdateShader");
    dataItem->waterShader = glLinkShader(vertexShader, fragmentShader);
    glDeleteObjectARB(vertexShader);
    glDeleteObjectARB(fragmentShader);
    dataItem->waterShaderUniformLocations[0] = glGetUniformLocationARB(
      dataItem->waterShader, "bathymetrySampler");
    dataItem->waterShaderUniformLocations[1] = glGetUniformLocationARB(
      dataItem->waterShader, "quantitySampler");
    dataItem->waterShaderUniformLocations[2] = glGetUniformLocationARB(
      dataItem->waterShader, "waterSampler");
  }
}
...
void WaterTable2::removeRenderFunction(
  const AddWaterFunction *removeRenderFunction) {
  /* Find the given render function in the list and remove it: */
  for (std::vector<const AddWaterFunction *>::iterator rfIt =
         renderFunctions.begin();
       rfIt != renderFunctions.end(); ++rfIt)
...
void WaterTable2::updateBathymetry(GLContextData &contextData) const {
  ...
  if (dataItem->bathymetryVersion !=
    ...
    /* Set up the integration frame buffer to update the conserved quantities
     * based on bathymetry changes: */
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,
      dataItem->integrationFramebufferObject);
    ...
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, currentFrameBuffer);
    glClearColor(currentClearColor[0], currentClearColor[1],
...
void WaterTable2::updateBathymetry(const GLfloat *bathymetryGrid,
  ...
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
    dataItem->bathymetryTextureObjects[1 - dataItem->currentBathymetry]);
  glUniform1iARB(dataItem->bathymetryShaderUniformLocations[1], 1);
  glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, size[0] - 1, size[1] - 1,
    GL_LUMINANCE, GL_FLOAT, bathymetryGrid);
...
void WaterTable2::setWaterLevel(const GLfloat *waterGrid,
  GLContextData &contextData) const {
  /* Get the data item: */
  DataItem *dataItem = contextData.retrieveDataItem<DataItem>(this);

  /* Set up the integration frame buffer to adapt the new water level to the
   * current bathymetry: */
  glPushAttrib(GL_VIEWPORT_BIT);
  GLint currentFrameBuffer;
  glGetIntegerv(GL_FRAMEBUFFER_BINDING_EXT, &currentFrameBuffer);
  ...
  glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, size[0], size[1], GL_RED,
    GL_FLOAT, waterGrid);
...
GLfloat WaterTable2::runSimulationStep(bool forceStepSize,
  ...
  *********************************************************************/
  ...
  GLfloat stepSize = calcDerivative(dataItem,
    dataItem->quantityTextureObjects[dataItem->currentQuantity],
    !forceStepSize);
  ...
  *********************************************************************/
  ...
  glUniformARB(dataItem->eulerStepShaderUniformLocations[1],
    Math::pow(attenuation, stepSize));
  glActiveTextureARB(GL_TEXTURE0_ARB);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
    dataItem->quantityTextureObjects[dataItem->currentQuantity]);
  ...
  *********************************************************************/
  ...
  glUniformARB(dataItem->rungeKuttaStepShaderUniformLocations[1],
    Math::pow(attenuation, stepSize));
  glActiveTextureARB(GL_TEXTURE0_ARB);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB,
    dataItem->quantityTextureObjects[dataItem->currentQuantity]);
  ...
  if (waterDeposit != 0.0f || !renderFunctions.empty()) {
    ...
    *******************************************************************/
    ...
    glUniformMatrix4fvARB(dataItem->waterAddShaderUniformLocations[0], 1,
      GL_FALSE, waterAddPmvMatrix);
    ...
    for (std::vector<const AddWaterFunction *>::const_iterator rfIt =
           renderFunctions.begin();
         rfIt != renderFunctions.end(); ++rfIt)
      (**rfIt)(contextData);
