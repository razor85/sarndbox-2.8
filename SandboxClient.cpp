...
void SandboxClient::TeleportTool::initNavState(void) {
  ...
  /* Set up a physical navigation frame around the main viewer's current head
   * position: */
  calcPhysicalFrame(headPos);

  /* Calculate the initial environment-aligned surface frame in navigation
   * coordinates: */
  surfaceFrame = Vrui::getInverseNavigationTransformation() * physicalFrame;
  Vrui::NavTransform newSurfaceFrame = surfaceFrame;

  /* Align the initial frame with the application's surface and calculate Euler
   * angles: */
  AlignmentData ad(surfaceFrame, newSurfaceFrame,
    Vrui::getMeterFactor() * Scalar(0.25), Vrui::getMeterFactor());
  Scalar elevation, roll;
  align(ad, azimuth, elevation, roll);

  /* Move the physical frame to the foot position, and adjust the surface frame
   * accordingly: */
  newSurfaceFrame *= Geometry::invert(physicalFrame) *
    Vrui::NavTransform::translate(footPos - headPos) * physicalFrame;
...
void SandboxClient::TeleportTool::initClass(void) {
  ...
  factory = new TeleportToolFactory("TeleportTool", "Teleport",
    Vrui::getToolManager()->loadClass("SurfaceNavigationTool"),
    *Vrui::getToolManager());
  ...
  Vrui::getToolManager()->addClass(factory,
...
SandboxClient::TeleportTool::TeleportTool(const Vrui::ToolFactory *factory,
  const Vrui::ToolInputAssignment &inputAssignment)
    : Vrui::SurfaceNavigationTool(factory, inputAssignment), cast(false) {
  ...
  case 1:
    if (isActive()) {
      if (cbData->newButtonState)
        ...
        surfaceFrame.leftMultiply(Vrui::NavTransform::translate(
          castArc.back() - surfaceFrame.getOrigin()));
...
void SandboxClient::TeleportTool::frame(void) {
  if (isActive()) {
    ...
    /* Transform the movement vector from physical space to the physical
     * navigation frame: */
    ...
    if (cast) {
      ...
      castArc.clear();
      Point cp = Vrui::getInverseNavigationTransformation().transform(
        getButtonDevicePosition(1));
      Vector cv = Vrui::getInverseNavigationTransformation().transform(
        getButtonDeviceRayDirection(1) * (Vrui::getMeterFactor() * Scalar(15)));
      Vector ca(0, 0,
        -Vrui::getInverseNavigationTransformation().getScaling() *
          Vrui::getMeterFactor() * Scalar(9.81));
      for (int i = 0; i < 100; ++i) {
...
void SandboxClient::TeleportTool::display(GLContextData &contextData) const {
  if (isActive() && cast) {
    ...
    glMaterialAmbientAndDiffuse(GLMaterialEnums::FRONT,
      GLColor<GLfloat, 4>(0.0f, 1.0f, 0.0f));
    glMaterialSpecular(GLMaterialEnums::FRONT,
      GLColor<GLfloat, 4>(0.333f, 0.333f, 0.333f));
    glMaterialShininess(GLMaterialEnums::FRONT, 32.0f);
    glMaterialEmission(GLMaterialEnums::FRONT,
      GLColor<GLfloat, 4>(1.0f, 0.0f, 0.0f));

    sphereRenderer.enable(Vrui::getNavigationTransformation().getScaling(),
      contextData);
    glBegin(GL_POINTS);
    for (std::vector<Point>::const_iterator caIt = castArc.begin();
         caIt != castArc.end(); ++caIt)
      glVertex4f((*caIt)[0], (*caIt)[1], (*caIt)[2], radius);
    glVertex4f(castArc.back()[0], castArc.back()[1], castArc.back()[2],
      Vrui::getMeterFactor() * Scalar(0.125) *
        Vrui::getInverseNavigationTransformation().getScaling());
    glEnd();
    sphereRenderer.disable(contextData);

    cylinderRenderer.enable(Vrui::getNavigationTransformation().getScaling(),
      contextData);
...
SandboxClient::DataItem::DataItem(void)
    : bathymetryTexture(0), waterTexture(0), textureVersion(0),
      bathymetryVertexBuffer(0), bathymetryIndexBuffer(0), waterVertexBuffer(0),
      waterIndexBuffer(0), bathymetryVertexShader(0),
      bathymetryFragmentShader(0), bathymetryShaderProgram(0),
...
SandboxClient::Scalar SandboxClient::intersectLine(
  const SandboxClient::Point &p0, const SandboxClient::Point &p1) const {
  /* Convert the points to grid coordinates: */
  Point gp0(p0[0] / Scalar(cellSize[0]) - Scalar(0.5),
    p0[1] / Scalar(cellSize[1]) - Scalar(0.5), p0[2]);
  Point gp1(p1[0] / Scalar(cellSize[0]) - Scalar(0.5),
    p1[1] / Scalar(cellSize[1]) - Scalar(0.5), p1[2]);
  Vector gd = gp1 - gp0;
  ...
  GLsizei cp[2];
  for (int i = 0; i < 2; ++i)
    cp[i] = Math::clamp(GLsizei(Math::floor(gp[i])), GLsizei(0),
      gridSize[i] - 3);
  Scalar cl0 = l0;
  while (cl0 < l1) {
    /* Calculate the line parameter where the line segment leaves the current
     * cell: */
    Scalar cl1 = l1;
    int exit = -1;
    for (int i = 0; i < 2; ++i) {
    ...
    const GLfloat *cell = grids.getLockedValue().bathymetry +
      (cp[1] * (gridSize[0] - 1) + cp[0]);
    Scalar c0 = cell[0];
    Scalar c1 = cell[1];
    ...
    if (a != Scalar(0)) {
      ...
      if (det >= Scalar(0)) {
        ...
        if (a > Scalar(0)) {
          ...
          il = b >= Scalar(0) ? (-b - det) / (Scalar(2) * a) :
                                (Scalar(2) * c) / (-b + det);
          if (il < cl0)
            il = b >= Scalar(0) ? (Scalar(2) * c) / (-b - det) :
                                  (-b + det) / (Scalar(2) * a);
        } else {
          /* Test the smaller intersection first: */
          il = b >= Scalar(0) ? (Scalar(2) * c) / (-b - det) :
...
bool SandboxClient::serverMessageCallback(
  Threads::EventDispatcher::ListenerKey eventKey, int eventType,
  void *userData) {
...
void SandboxClient::alignSurfaceFrame(
  Vrui::SurfaceNavigationTool::AlignmentData &alignmentData) {
  ...
  GLsizei gx = Math::clamp(GLsizei(Math::floor(dx)), GLsizei(0),
    gridSize[0] - GLsizei(3));
  dx -= gx;
  Scalar dy = base[1] / Scalar(cellSize[1]) - Scalar(0.5);
  GLsizei gy = Math::clamp(GLsizei(Math::floor(dy)), GLsizei(0),
    gridSize[1] - GLsizei(3));
  dy -= gy;
  ...
  alignmentData.surfaceFrame = Vrui::NavTransform(base - Point::origin,
    Vrui::Rotation::identity, alignmentData.surfaceFrame.getScaling());
}
...
void SandboxClient::compileShaders(SandboxClient::DataItem *dataItem,
  ...
  for (int lightIndex = 0; lightIndex < lightTracker.getMaxNumLights();
       ++lightIndex)
    if (lightTracker.getLightState(lightIndex).isEnabled()) {
      /* Create the light accumulation function: */
      bathymetryVertexShaderFunctions += lightTracker
                                           .createAccumulateLightFunction(
                                             lightIndex);

      /* Call the light application function from the bathymetry vertex shader's
       * main function: */
      ...
      bathymetryVertexShaderMain +=
        "(vertexEc,normalEc,gl_FrontMaterial.ambient,gl_FrontMaterial.diffuse,"
        "gl_FrontMaterial.specular,gl_FrontMaterial.shininess,ambDiff,spec);\n";
    }
  ...
  glCompileShaderFromStrings(dataItem->bathymetryVertexShader, 5,
    bathymetryVertexShaderDefines.c_str(),
    bathymetryVertexShaderFunctions.c_str(),
    bathymetryVertexShaderUniforms.c_str(),
    bathymetryVertexShaderVaryings.c_str(), bathymetryVertexShaderMain.c_str());
  ...
  dataItem->bathymetryShaderUniforms[0] = glGetUniformLocationARB(
    dataItem->bathymetryShaderProgram, "bathymetrySampler");
  dataItem->bathymetryShaderUniforms[1] = glGetUniformLocationARB(
    dataItem->bathymetryShaderProgram, "bathymetryCellSize");
  dataItem->bathymetryShaderUniforms[2] = glGetUniformLocationARB(
    dataItem->bathymetryShaderProgram, "waterColor");
  dataItem->bathymetryShaderUniforms[3] = glGetUniformLocationARB(
    dataItem->bathymetryShaderProgram, "waterOpacity");
  ...
  for (int lightIndex = 0; lightIndex < lightTracker.getMaxNumLights();
    ...
    if (lightTracker.getLightState(lightIndex).isEnabled()) {
      /* Create the light accumulation function: */
      waterVertexShaderFunctions += lightTracker.createAccumulateLightFunction(
        lightIndex);
  ...
  glCompileShaderFromStrings(dataItem->waterVertexShader, 4,
    waterVertexShaderDefines.c_str(), waterVertexShaderFunctions.c_str(),
    waterVertexShaderUniforms.c_str(), waterVertexShaderMain.c_str());
  ...
  glCompileShaderFromString(dataItem->waterFragmentShader,
    waterFragmentShaderMain.c_str());
  ...
  dataItem->waterShaderUniforms[0] = glGetUniformLocationARB(
    dataItem->waterShaderProgram, "bathymetrySampler");
  dataItem->waterShaderUniforms[1] = glGetUniformLocationARB(
    dataItem->waterShaderProgram, "waterSampler");
  dataItem->waterShaderUniforms[2] = glGetUniformLocationARB(
    dataItem->waterShaderProgram, "waterCellSize");
...
SandboxClient::SandboxClient(int &argc, char **&argv)
    : Vrui::Application(argc, argv), pipe(0), gridVersion(0), sun(0),
      underwater(false) {
  ...
  for (int argi = 1; argi < argc; ++argi) {
    if (argv[argi][0] == '-') {
      std::cerr << "SandboxClient: Ignoring command line option " << argv[argi]
                << std::endl;
    } else if (serverName == 0)
      serverName = argv[argi];
    else
      std::cerr << "SandboxClient: Ignoring command line argument "
                << argv[argi] << std::endl;
  }
  if (serverName == 0)
  ...
  if (token == 0x78563412U)
    ...
    throw std::runtime_error(
      "SandboxClient: Invalid response from remote AR Sandbox");
  }

  try {
    /* Receive the remote AR Sandbox's water table grid size, cell size, and
     * elevation range: */
    for (int i = 0; i < 2; ++i) {
  ...
  dispatcher.addIOEventListener(pipe->getFd(), Threads::EventDispatcher::Read,
    serverMessageCallback, this);
  ...
  Vrui::getCoordinateManager()->setUnit(
    Geometry::LinearUnit(Geometry::LinearUnit::METER, 0.01));
...
void SandboxClient::display(GLContextData &contextData) const {
  ...
  glMaterialAmbientAndDiffuse(GLMaterialEnums::FRONT,
    GLColor<GLfloat, 4>(0.6f, 0.4f, 0.1f));
  glMaterialSpecular(GLMaterialEnums::FRONT,
  ...
  if (dataItem->textureVersion != gridVersion) {
    ...
    glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, gridSize[0] - 1,
      gridSize[1] - 1, GL_RED, GL_FLOAT, grids.getLockedValue().bathymetry);
  }
  ...
  glUniform2fARB(dataItem->bathymetryShaderUniforms[1], cellSize[0],
    cellSize[1]);
  glUniform4fARB(dataItem->bathymetryShaderUniforms[2], 0.2f, 0.5f, 0.8f, 1.0f);
  glUniform1fARB(dataItem->bathymetryShaderUniforms[3],
    underwater ? 0.1f : 0.0f);
  ...
  /* Draw the bathymetry: */
  {
    ...
    GLuint *indexPtr = 0;
    for (GLsizei y = 1; y < gridSize[1] - 1;
         ++y, indexPtr += (gridSize[0] - 1) * 2)
      glDrawElements(GL_QUAD_STRIP, (gridSize[0] - 1) * 2, GL_UNSIGNED_INT,
        indexPtr);
  ...
  glMaterialAmbientAndDiffuse(GLMaterialEnums::FRONT,
    GLColor<GLfloat, 4>(0.2f, 0.5f, 0.8f));
  glMaterialSpecular(GLMaterialEnums::FRONT,
    GLColor<GLfloat, 4>(1.0f, 1.0f, 1.0f));
  ...
  if (dataItem->textureVersion != gridVersion) {
    ...
    glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, gridSize[0], gridSize[1],
      GL_RED, GL_FLOAT, grids.getLockedValue().waterLevel);
  }
...
void SandboxClient::initContext(GLContextData &contextData) const {
  ...
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F, gridSize[0] - 1,
    gridSize[1] - 1, 0, GL_RED, GL_FLOAT, 0);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, 0);
  ...
  glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S, GL_CLAMP);
  glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T, GL_CLAMP);
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_R32F, gridSize[0], gridSize[1],
    0, GL_RED, GL_FLOAT, 0);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, 0);
  ...
  /* Upload the grid of bathymetry template vertices into the vertex buffer: */
  {
    ...
    glBufferDataARB(GL_ARRAY_BUFFER_ARB,
      (gridSize[1] - 1) * (gridSize[0] - 1) * sizeof(Vertex), 0,
      GL_STATIC_DRAW_ARB);
    Vertex *vPtr = static_cast<Vertex *>(
    ...
    glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
      dataItem->bathymetryIndexBuffer);
    glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
      (gridSize[1] - 2) * (gridSize[0] - 1) * 2 * sizeof(GLuint), 0,
      GL_STATIC_DRAW_ARB);
    GLuint *iPtr = static_cast<GLuint *>(
   ...
   */
  {
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, dataItem->waterVertexBuffer);
    glBufferDataARB(GL_ARRAY_BUFFER_ARB,
      gridSize[1] * gridSize[0] * sizeof(Vertex), 0, GL_STATIC_DRAW_ARB);
    Vertex *vPtr = static_cast<Vertex *>(
      glMapBufferARB(GL_ARRAY_BUFFER_ARB, GL_WRITE_ONLY_ARB));
    for (GLsizei y = 0; y < gridSize[1]; ++y)
    ...
    glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, dataItem->waterIndexBuffer);
    glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
      (gridSize[1] - 1) * gridSize[0] * 2 * sizeof(GLuint), 0,
      GL_STATIC_DRAW_ARB);
    GLuint *iPtr = static_cast<GLuint *>(
      glMapBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, GL_WRITE_ONLY_ARB));
